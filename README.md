...
> 给你的 AI 编程工具配一个 DeepSeek 参谋。同一个错误不犯第二次。
...
Sonnet 便宜但容易反复踩坑，Opus 强但贵。kaiwu 给中低端模型挂上**错误库 + 经验库 + 知识库**，让它们少走弯路、不重复犯错，用更少的 token 完成更多任务。
...
```
错误发生 → 指纹精确匹配（0.49ms）→ 关键词模糊匹配 → DeepSeek 深度分析
                ↓                        ↓                    ↓
...
```
任务执行 → 轨迹审计（有转折？踩过坑？）→ DeepSeek 提炼方法论 → 存入经验库
                                                                    ↓
...
- 弱模型（Haiku/Mini）才启用 DeepSeek 全力辅助
- 连续 2 次同类错误自动检测循环，建议换方向，**避免 token 黑洞**
- 日均 DeepSeek 消耗约 **¥0.1**（新用户赠送 500 万 tokens）
...
| 任务分类器 | **0.09ms** | 0 | 自动判断是否需要 DeepSeek |
...
| DeepSeek 规划 | ~2-5s | ~600 | 仅中低端模型触发 |
| DeepSeek 诊断 | ~2-5s | ~400 | 仅本地未命中时触发 |
...
**7 个 MCP 工具**（自动注册，无需手动配置）：

| 工具 | 作用 | 触发时机 |
|------|------|------|
| `kaiwu_plan` | 任务规划 + 陷阱预警 | 接到新任务 |
| `kaiwu_lessons` | 三层错误诊断 | 遇到报错 |
| `kaiwu_record` | 经验记录 + 轨迹审计 | 任务完成 |
| `kaiwu_context` | 项目上下文管理 | 任务开始 |
| `kaiwu_condense` | 长对话压缩 | 超 15 轮 |
| `kaiwu_scene` | 场景编码规范 | 按需 |
| `kaiwu_profile` | 用户习惯画像 | 按需 |
